...
CREATE POLICY "seo_intelligence_public_select" ON public.seo_intelligence FOR SELECT USING (true);
CREATE POLICY "seo_intelligence_service_role_all" ON public.seo_intelligence FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

-- 3. SEMANTIC KEYWORDS TABLE
...
CREATE POLICY "semantic_keywords_public_select" ON public.semantic_keywords FOR SELECT USING (true);
CREATE POLICY "semantic_keywords_service_role_all" ON public.semantic_keywords FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

-- 4. COMPETITOR GAPS TABLE
...
CREATE POLICY "competitor_gaps_public_select" ON public.competitor_gaps FOR SELECT USING (true);
CREATE POLICY "competitor_gaps_service_role_all" ON public.competitor_gaps FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

-- 5. PAA QUESTIONS TABLE
...
CREATE POLICY "paa_questions_public_select" ON public.paa_questions FOR SELECT USING (true);
CREATE POLICY "paa_questions_service_role_all" ON public.paa_questions FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

-- 6. CONTENT ANALYTICS TABLE
CREATE TABLE IF NOT EXISTS public.content_analytics (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  county_id BIGINT REFERENCES public.counties(id) ON DELETE CASCADE,
  semantic_coverage_percent DECIMAL(5,2),
  competitor_gaps_filled INTEGER,
  competitor_gaps_total INTEGER,
  paa_coverage_count INTEGER,
...
CREATE POLICY "content_analytics_public_select" ON public.content_analytics FOR SELECT USING (true);
CREATE POLICY "content_analytics_service_role_all" ON public.content_analytics FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

-- 7. UPDATE EXISTING COUNTY DATA
UPDATE public.counties SET
