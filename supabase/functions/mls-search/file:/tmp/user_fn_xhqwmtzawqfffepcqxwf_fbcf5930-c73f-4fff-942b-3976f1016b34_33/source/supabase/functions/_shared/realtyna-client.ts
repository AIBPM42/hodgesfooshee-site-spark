...
export function getRealtynaBaseUrl() {
...
export function getRealtynaHeaders(token) {
  const apiKey = Deno.env.get('REALTYNA_API_KEY');
  if (!apiKey) {
    throw new Error('ENV_MISSING: REALTYNA_API_KEY is required');
  }
  const origin = Deno.env.get('PUBLIC_SITE_ORIGIN') || Deno.env.get('SUPABASE_URL') || 'https://preview.lovable.app';
